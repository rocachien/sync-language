...
const stringEncrypt = (text) => {
    text = text.replace(/\\/g, '__0001__');
    text = text.replace(/\//g, '__0002__');
    text = text.replace(/\?/g, '__0003__');
    text = text.replace(/\!/g, '__0004__');
    return text;
};

const stringDecrypt = (text) => {
    text = text.replace(/__0001__/g, '\\');
    text = text.replace(/__0002__/g, '/');
    text = text.replace(/__0003__/g, '?');
    text = text.replace(/__0004__/g, '!');
    return text;
};
...
sync.updateFromLocal = async () => {
    ...
    try {
        ...
        if (lcData) {
            ...
            lcData = stringEncrypt(lcData);
            await sync.write(conf.fileDB, lcData);
            ...
            if (localize && localize.en && localize.vi) {
                ...
                Object.keys(localize.vi).forEach(function (key) {
                    ...
                    const originKey = stringDecrypt(prefix ? key.replace(prefix, '') : key);

                    o.ResourceID = originKey;
                    o.LocalEnglish = stringDecrypt(enLang ? enLang : viLang);
                    o.LocalVietNamese = stringDecrypt(viLang ? viLang : enLang);
...
sync.updateFromServer = async () => {
    ...
    if (result && result.recordset) {
        for (let i = 0; i < result.recordset.length; i++) {
            ...
            o.ListProductsIDs = l.ListProductsIDs ? l.ListProductsIDs : "";
...
sync.checkServerChange = (language) => {
    ...
    let found = false;
    let isUpdate = false;
    const productID = conf.productID;
    ...
    for (let i = 0; i < conf.syncData.length ; i++) {
        ...
        if (o.ResourceID === language.ResourceID) {
            ...
            if (!language.ListProductsIDs.includes(productID)) {
                isUpdate = true;
                conf.syncData[i].IsUpdate = 1;
            }
        }
    }
...
sync.applySyncToServer = async () => {
    ...
    for (let i = 0; i < conf.syncData.length; i++) {
        ...
        } else  if (o.LocalTime > o.ServerTime || o.IsUpdate) {
